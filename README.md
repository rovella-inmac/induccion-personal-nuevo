...
- **Video embebido** de Vimeo en la diapositiva 06, con autoplay y sonido automático.
...
Abrir la URL publicada del repositorio. El video (Vimeo) con sonido funciona
correctamente porque se sirve sobre HTTPS.

### Local
Servir la carpeta con un servidor HTTP (no abrir `index.html` directo desde el disco,
ya que el video de Vimeo requiere un origen HTTP válido):
...
- El video usa el **SDK de Vimeo** (`player.js`). El reproductor se "desbloquea" con el
  primer clic en *Reproducir* para permitir autoplay **con sonido** sin clics adicionales.
...
- **Cambiar el video:** editar la URL del `<iframe id="vimeoPlayer">` en `index.html`.
